...
library.init = async function (params) {
	...
	routeHelpers.setupAdminPageRoute(router, '/admin/plugins/vsk-community', [], controllers.renderAdminPage);
};

library.addAdminNavigation = async function (header) {
	header.plugins.push({
		route: '/plugins/vsk-community',
		icon: 'fa-paint-brush',
		name: 'Vsk Community Theme',
	});
	return header;
};
...
library.getThemeConfig = async function (config) {
	const { enableQuickReply } = await meta.settings.get('vsk-community');
...
library.removeFinalBreadcrumb = async (hookData) => {
	...
	// Remove the last breadcrumb (the current page) as it is not part of Vsk Community's design
